...
PRICES = [
    LabeledPrice(label='Ноутбук', amount=100000000),
    LabeledPrice(label='Прочная упаковка', amount=10000000)
]

SUPERSPEED_SHIPPING_OPTION = ShippingOption(
    id='superspeed',
    title='Супер быстрая!'
).add(LabeledPrice('Лично в руки!', 10000000))

POST_SHIPPING_OPTION = ShippingOption(
    id='post',
    title='Почта Узбекистана'
)

POST_SHIPPING_OPTION.add(LabeledPrice('Кортонная коробка', 5000000))
POST_SHIPPING_OPTION.add(LabeledPrice('Срочное отправление!', 10000000))

PICKUP_SHIPPING_OPTION = ShippingOption(
    id='pickup',
    title='Самовывоз'
)
PICKUP_SHIPPING_OPTION.add(LabeledPrice('Самовывоз в Фергане', 500000))
...
async def shipping_process(shipping_query: ShippingQuery):
    ...
    if shipping_query.shipping_address.country_code == 'UZ':
        ...
        if shipping_query.shipping_address.city == 'Fergana':
